...
def retrieve_from_vector_database(user_input, user_location):
...
def retrieve_with_location(input_dict):
    ...
    return retrieve_from_vector_database(user_input, user_location)

chain = (
    {"context": retrieve_with_location, "question": RunnablePassthrough()}
    | prompt
    | model
    | StrOutputParser()
)
...
def responsetext(user_input, user_location):
    result = chain.invoke({"question": user_input, "location": user_location})
